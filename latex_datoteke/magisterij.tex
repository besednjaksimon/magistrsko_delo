...
\begin{trditev}
	Naj bosta polinoma $\balpha$ in $\bbeta$ taka, da je $\bgamma$ stopnje $r=1$ ter naj generirata tako dvojno PH krivuljo, da je polinom $h$ druge stopnje in da je polinom $\wV$ linearen. Potem je ta dvojna PH krivulja vijačna.
\end{trditev}
\begin{proof}
...
\begin{trditev}
	Naj bosta polinoma $\balpha$ in $\bbeta$ taka, da je $\bgamma$ stopnje $r=0$ ter naj generirata tako dvojno PH krivuljo, da je polinom $h$ druge stopnje in da je polinom $\wV$ linearen. Potem je taka dvojna PH krivulja nevijačna.
\end{trditev}
\begin{proof}
...
\begin{trditev}
	...
	\end{equation}
	ki je podan z Bernsteinovimi koeficienti polinoma $h,$ negativen.
\end{trditev}
\begin{proof}
...
V tem poglavju si bomo na podlagi do zdaj znanih dejstev ogledali, kako konstruirati prostorsko DPH krivuljo stopnje 7. Obdelali bomo dva primera, eno vijačno ter eno nevijačno krivuljo.
...
\begin{primer}[Vijačna krivulja, generirana preko množenja s kvadratičnim polinomom]
	...
	\end{equation*}
	Polinom $\wV$ podamo preko Bernsteinovih koeficientov $\wV_0=1,$ $\wV_1=1+\iu$ in $\wV_2=1.$ Števec in imenovalec izraza $\zV(t)$ množimo z $\wV(t),$ dobimo $\zV(t)=\balpha(t)/\bbeta(t).$ Potem se Bernsteinove koeficiente polinomov $\balpha(t)$ in $\bbeta(t)$ izračuna preko enačb \eqref{mnozenje_kvadraticni_polinom_7_koef1} in \eqref{mnozenje_kvadraticni_polinom_7_koef2}. Enaki so
	\begin{align*}
	...
	% \caption[caption za v kazalo]{Dolg caption pod sliko}
	  \caption[Primer vijačne krivulje.]{Na sliki je razviden graf vijačne krivulje s pripadajočim kontrolnim poligonom, ki smo jo pridobili z množenjem števca in imenovalca izraza \eqref{transformacija} s polinomom $\wV.$}
...
\begin{primer}[$\st(h)=4,$ $\st(\wV)=0,$ Nevijačna]
	Konstruirali bomo tak tip krivulje, kot smo ga analizirali v poglavju \ref{klasifikacija_h4w0}. Za začetek si izberemo naslednja polinoma $h$ in $\wV,$ izražena v Bernsteinovi bazi z izbiro koeficientov
	\begin{equation*}
		h_0=1,\quad h_1=2,\quad h_2=2,\quad h_3=2,\quad h_4=1,\quad \wV_0=1.
	\end{equation*}
	Vidimo, da je količina $\Delta=9h_2^2+3h_0h_4-12h_1h_4=-9$ negativna in po trditvi \ref{trditev_locevanje_h4w0} sledi, da je krivulja, ki jo generiramo, nevijačna. Prosto si izberemo še vrednosti koeficientov $\balpha_0=1,$ $\balpha_1=1$ in $\bbeta_0=-1.$ Nato rešimo bilinearni sistem enačb \eqref{sistem_st7h4w0} in pri tem upoštevamo rešitev, ki je negativno predznačena pri $\zV=\balpha_1\bbeta_2-\balpha_2\bbeta_1$ v enačbi \eqref{st7h4w0}. Sistem lahko rešimo kar s pomočjo programa \emph{Mathematica}. Tako dobimo naslednje Bernsteinove koeficiente za polinoma $\balpha$ in $\bbeta:$
	\begin{align*}
	...
	Za integracijsko konstanto si kot prej izberemo točko $\pV_0=(0,0,0).$ Ostale kontrolne točke krivulje izračunamo preko enačb \eqref{kontrolne_tocke}. Enake so
	\begin{align*}
